                    ...
                    with st.spinner("⏳ 正在利用最新視覺模型解構您的 LINE 截圖..."):
                        ...
                        if detected_group and not matched_group_success:
                            ...
                            def show_client_not_found_dialog(detected_grp):
                                st.markdown(f"🤖 **AI 智慧分析偵測**：本次圖片中抓到客戶特徵為：『**{detected_grp}**』，但資料庫中找不到對應。")
                                ...
                                with tab2:
                                    new_c_id = st.text_input("新客戶編號 (必須唯一)：", key="dialog_new_c_id").strip()
                                    new_c_name = st.text_input("新客戶標準官方全名：", key="dialog_new_c_name").strip()
                                    if st.button("🚀 確認建立新客戶並直接帶入對帳", use_container_width=True):
                                        ...
                                        else: st.error("❌ 編號與名稱皆為必填項目！")
                            show_client_not_found_dialog(detected_group)
                        else:
                            st.success("🎉 圖片解構完成！已將品項轉換並帶入下方的拆解暫存區！")
                            time.sleep(0.5)
                            st.rerun()
