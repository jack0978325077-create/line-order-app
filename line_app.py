...
# ==============================================================================
# 🔐 2. 雲端環境安全連線設定區 (內建雙棲防呆與連線錯誤捕獲)
# ==============================================================================
# 優先讀取 Streamlit Secrets 後台，若無則讀取你原來的寫死設定
SUPABASE_URL = st.secrets.get("SUPABASE_URL", "https://ktmepyfafstgxklrwhoq.supabase.co")
SUPABASE_KEY = st.secrets.get("SUPABASE_KEY", "sb_publishable_ROyxuswMSHsq0uymo9UVyw_K1qM3MY0")
...
try:
    if SUPABASE_URL and SUPABASE_KEY:
        ...
        st.error("""
            ❌ **資料庫鑰匙缺失！** * 💡 **可能原因**：雲端 Secrets 後台未設定 `SUPABASE_URL` 或 `SUPABASE_KEY`。  
            * 🛠️ **解決辦法**：請管理員立刻進入 Streamlit 後台的 Settings ➡️ Secrets 補上連線金鑰。
        """)
except Exception as e:
    st.error(f"""
        ❌ **雲端資料庫（Supabase）初始化崩潰！** * 💡 **錯誤原因**：{str(e)}  
        * 🛠️ **解決辦法**：請檢查密鑰是否複製完整。若金鑰無誤，可能是 Supabase 雲端正進行維護，請稍候 1 分鐘重整網頁。
    """)

# ==============================================================================
# 🧠 3. 初始化 Session State 記憶體變數
# ==============================================================================
...
if "unified_text_val" not in st.session_state: st.session_state["unified_text_val"] = ""
if "img_run_counter" not in st.session_state: st.session_state["img_run_counter"] = 0
if "ai_detected_group_name" not in st.session_state: st.session_state["ai_detected_group_name"] = ""
...
# ==============================================================================
# 🎯 5. 神級功能：客戶比對失敗自動彈出中央互動視窗 (st.dialog)
# ==============================================================================
@st.dialog("⚠️ 未能識別有效的對帳客戶")
def show_client_not_found_dialog(detected_group, all_customers_list, search_options_list, cust_mapping_dict):
    st.markdown(f"🤖 **AI 智慧分析偵測**：本次圖片/文字中抓到客戶特徵為：『**{detected_group}**』，但目前資料庫查無此綁定。")
    st.write("請選擇現有客戶進行永久特徵綁定，或直接在此建立全新客戶：")
    
    tab1, tab2 = st.tabs(["🔗 綁定到現有客戶", "➕ 建立全新客戶並直接帶入"])
    
    with tab1:
        learn_select = st.selectbox("🎯 請指定此特徵歸屬於哪位正式客戶：", options=search_options_list, key="dialog_learn_box")
        if st.button("💾 確認綁定，並讓系統永遠記住", use_container_width=True, key="btn_dialog_link_old"):
            if learn_select == "請選擇或輸入文字搜尋客戶... (留空代表查詢當日全廠)":
                st.error("❌ 請先在下拉選單選擇一位正式客戶！")
            else:
                try:
                    target_cust = cust_mapping_dict[learn_select]["raw_data"]
                    ...
                    st.success(f"🎉 學習成功！系統已將『{detected_group}』永久綁定給【{target_cust['standard_name']}】！")
                    time.sleep(0.8)
                    st.rerun()
                except Exception as le:
                    st.error(f"❌ **特徵寫入資料庫失敗**：{str(le)}。請檢查雲端資料庫狀態。")
    ...
    with tab2:
        ...
        if st.button("🚀 確認建立新客戶並直接帶入對帳", use_container_width=True, key="btn_dialog_create_new"):
            if not new_c_id or not new_c_name:
                st.error("❌ **防呆警告**：新客戶的編號與名稱皆為必填項目，不能留空！")
            elif not supabase_connected:
                st.error("❌ **無法新增**：資料庫未成功連線，拒絕寫入。")
            else:
                try:
                    # 建立新客戶並把當前學到的特徵作為起步關鍵字
                    combined_kw = f"SHORTCUT: , {detected_group}"
                    supabase.table("customers").insert({
                        "customer_id": new_c_id,
                        "standard_name": new_c_name,
                        "search_keywords": combined_kw
                    }).execute()
                    
                    st.session_state["final_c_name"] = new_c_name
                    st.session_state["final_c_id"] = new_c_id
                    st.session_state["ai_detected_group_name"] = ""
                    st.success(f"🎉 成功新增客戶『{new_c_name}』並已直接為您帶入系統！")
                    time.sleep(0.8)
                    st.rerun()
                except Exception as ex:
                    st.error(f"❌ **新客戶建立失敗**：{str(ex)}。請檢查客戶編號是否跟以前重複了。")
...
if db_mode == "Line圖片文字叫貨":  
    ...
    if supabase_connected:
        try:
            ...
            st.error(f"⚠️ **撈取資料庫客戶清單失敗**：{str(e)}，系統將暫時切換至單機運作。")
    ...
    if input_mode == "📸 圖片截圖上傳模式":
        ...
        if uploaded_file:
            if st.button("⚡ 開始執行圖片 AI 智慧拆解並帶入暫存區", key="btn_img_go", use_container_width=True):
                if not api_key:
                    st.error("❌ **AI 啟動失敗**：找不到您的 Gemini API Key！請檢查左側側邊欄或雲端 Secrets 是否填寫完整。")
                else:
                    try:
                        ...
                        with st.spinner("⏳ 正在利用最新視覺模型解構您的 LINE 截圖..."):
                            ...
                            # 💥 核心防呆彈窗觸發：若有群組名稱但資料庫比對失敗，立刻蹦出彈窗
                            if detected_group and not matched_group_success:
                                show_client_not_found_dialog(detected_group, all_customers, search_options, cust_mapping)
                            else:
                                st.success("🎉 圖片解構完成！已自動對應熟客，並將品項帶入下方暫存區！")
                                time.sleep(0.5)
                                st.rerun()
                    except Exception as img_err:
                        st.error(f"""
                            ❌ **圖片視覺解析崩潰！** * 💡 **可能原因**：{str(img_err)}  
                            * 🛠️ **解決辦法**：請檢查您的 API Key 是否過期或輸入錯誤。若金鑰正常，可能是截圖檔案太大，請試著縮小圖片尺寸後重新上傳。
                        """)
        ...
        if unified_text.strip() and not has_customer:
            st.warning("⚠️ **【對帳卡關攔截】** AI 已順利抓出品項，但目前『尚未選定對帳客戶』！\n👉 **請立刻滑動到網頁最上方，手動在下拉選單指定正式客戶**，下方的功能按鈕才會解鎖點亮。")
            ...
            with btn_col1:
                ...
                if st.button("📦 這些品項均『登記需出貨品項』(存入雲端庫存)", key="btn_all_ship_go", use_container_width=True, disabled=not has_customer):
                    if not supabase_connected:
                        st.error("❌ **登記失敗**：系統目前與雲端資料庫失去了連線，無法儲存。請重整網頁。")
                    else:
                        try:
                            client = genai.Client(api_key=api_key)
                            with st.spinner("⏳ 正在整合歷史未出舊帳，並將今日品項儲存至雲端..."):
                                ...
                                st.session_state["img_run_counter"] += 1
                                st.success("🎉 歷史舊帳與今日新單已完美合併！下方已同步刷新最新蓄水池總量！")
                                time.sleep(0.5)
                                st.rerun()
                        except Exception as err: 
                            st.error(f"❌ **登記程序執行崩潰**：{str(err)}。請檢查品項大框框內是否有不合規的字元。")
                            
            with btn_col2:
                # 【按鈕防呆：圖片模式 B檔一鍵核銷】
                if st.button("❌ 這些品項『目前無出貨』，其餘品項均出貨", key="btn_remain_b_go", use_container_width=True, disabled=not has_customer):
                    if not supabase_connected:
                        st.error("❌ **核銷失敗**：系統目前與雲端資料庫斷開，請重整網頁。")
                    else:
                        try:
                            client = genai.Client(api_key=api_key)
                            with st.spinner("⏳ 正在撈取歷史蓄水池，極速全自動核銷並生成 Excel 中..."):
                                ...
                                for prod_name in pool_dict.keys():
                                    supabase.table("delivery_orders").delete().eq("customer_name", st.session_state["final_c_name"]).eq("product_name", prod_name).eq("status", "已登記未出貨").eq("delivery_date", final_date).execute()
                                ...
                                for prod_name, total_pool_qty in pool_dict.items():
                                    ...
                                    action_text = "B檔留空➔全出" if matched_b_qty == 0 else f"欠貨剩餘: {matched_b_qty}"
                                    pr_info = p_dict.get(prod_name, {})
                                    table_rows_preview.append({
                                        "商品編號": pr_info.get("product_id", "新編號"), "商品名稱": prod_name, "雲端累積總量": total_pool_qty,
                                        "B檔未發剩餘": matched_b_qty, "實際出貨數量": actual_ship, "單價": float(pr_info.get("price", 0.0)),
                                        "總金額": actual_ship * float(pr_info.get("price", 0.0)), "核銷動作": action_text
                                    })
                                ...
                                st.session_state["unified_text_val"] = ""
                                st.session_state["img_run_counter"] += 1
                                st.success("🎉 核銷定格成功！出貨與欠貨軌跡已完美入庫！請直接在下方下載 Excel。")
                                time.sleep(0.5)
                                st.rerun()
                        except Exception as err: 
                            st.error(f"❌ **一鍵核銷執行失敗**：{str(err)}。原因可能是雲端蓄水池內並無該客戶的登記品項。")
    ...
    # ==================== ✍️ 純文字複製貼上模式 ====================
        ...
        if (text_a or text_b):
            if st.button("⚡ 開始執行純文字智慧拆解分析", key="btn_txt_go", use_container_width=True):
                if not api_key:
                    st.error("❌ **AI 啟動失敗**：請先填寫 Gemini API Key 金鑰密碼。")
                elif not has_customer:
                    st.error("❌ **防呆警告**：請先在最上方下拉選單選擇客戶，才能進行文字拆解！")
                else:
                    try:
                        client = genai.Client(api_key=api_key)
                        ...
                        # 如果文字中抓到的群組與當前選的不同，跳出學特徵彈窗
                        ai_grp = st.session_state.get("ai_detected_group_name", "")
                        if ai_grp and ai_grp != clean_string(st.session_state["final_c_name"]):
                            show_client_not_found_dialog(ai_grp, all_customers, search_options, cust_mapping)
                        else:
                            st.success("🎉 純文字雙線道解構分析完畢！數據已同步至雲端核心！")
                            time.sleep(0.5)
                            st.rerun()
                    except Exception as txt_err:
                        st.error(f"❌ **文字智慧拆解失敗**：{str(txt_err)}。請確認文字格式是否正確。")
        ...
        try:
            ...
            for prod_name, total_pool_qty in pool_dict.items():
                ...
                if st.session_state["is_ai_mode"] or len(items_b) > 0:
                    actual_ship = max(0, total_pool_qty - matched_b_qty)
                    action_note = "B檔留空➔全出" if matched_b_qty == 0 else (f"❌ 現場全無到貨" if actual_ship == 0 else f"⚠️ 部分到貨(出{actual_ship}/欠{matched_b_qty})")
                else:
                    actual_ship = 0
            ...
            st.error(f"❌ **核心扣帳計算模組異常**：{str(calc_err)}。請確認雲端資料庫欄位是否被改動。")
                        ...
                        try:
                            with st.spinner("⏳ 正在將出貨數據定格寫入今日歷史，並處理欠貨結轉..."):
                                ...
                                for _, r in df_final.iterrows():
                                    ...
                                    # 生成定格新欠單
                                    if rem_qty > 0:
                                        supabase.table("delivery_orders").insert({"delivery_date": final_date, "customer_name": st.session_state["final_c_name"], "product_name": p_name, "quantity": rem_qty, "status": "聯絡處理" if False else "已登記未出貨"}).execute()
                                ...
                                for _, r in st.session_state[state_key].iterrows():
                                    ...
                                    if final_ship_qty > 0:
                                        ...
                                        excel_rows.append({
                                            ...
                                            "客戶名稱": str(st.session_state["final_c_name"]), "日期": str(final_date),
                                            "商品編號": str(pr_info.get("product_id", "新編號")), "商品名稱": str(pr_name),
                            ...
                            st.error(f"❌ **結帳結轉崩潰**：{str(checkout_err)}。請通知管理員。")
        else:
            st.warning("⚠️ 提示：當前處於純瀏覽狀態。如需核銷，請取消勾選歷史累計並指定正確客戶與單日日期。")
    else:
...
# ==============================================================================
    ...
    def show_order_detail(c_name, c_date, target_status):
        ...
        except Exception as e: st.error(f"明細讀取失敗: {str(e)}")
    ...
    try:
        ...
        st.error(f"❌ **日曆渲染出錯**：{str(calendar_err)}")
...
# ==============================================================================
    ...
    with st.expander("📥 批次匯入新版商品 Excel 檔案", expanded=True):
        ...
        if p_excel_file:
            try:
                ...
                st.info(f"📋 系統成功讀取！此 Excel 中包含有編號及品名的有效商品共有 **{len(valid_rows)}** 筆。")
                st.dataframe(df_p_import.head(5))
                
                if len(df_p_import.columns) >= 7:
                    # 【按鈕防呆：Excel匯入】
                    if st.button("🚀 確認將上方全數商品匯入雲端 (鎖定 G 欄批價)"):
                        if not supabase_connected:
                            st.error("❌ **匯入失敗**：資料庫未成功連線。")
                        else:
                            success_p_count = 0
                            new_product_count = 0
                                ...
                                if p_id and p_name and p_id != "nan" and p_name != "nan" and p_id.lower() != "null":
                                    ...
                                    try:
                                        supabase.table("product_master").upsert({
                                            ...
                                            "unit": final_unit_combined, "price": p_price
                                        }).execute()
                                        success_p_count += 1
                    ...
                    st.error("❌ **匯入失敗**：您的 Excel 欄位架構不足，找不到 G 欄（成本/批價）。")
            except Exception as e:
                st.error(f"❌ **讀取商品 Excel 發生崩潰**: {str(e)}")
    ...
    with col_p1:
        with st.form("manual_product_form", clear_on_submit=True):
            ...
            # 【按鈕防呆：手動新增商品】
            if submit_manual_p:
                if not manual_id or not manual_name:
                    st.error("❌ **防呆警告**：商品編號與名稱皆為必填項目！")
                elif not supabase_connected:
                    st.error("❌ **無法儲存**：資料庫未成功連線。")
                else:
                    try:
                        supabase.table("product_master").upsert({
                            "product_id": manual_id, "product_name": manual_name,
                            "unit": manual_unit, "price": manual_price
                        }).execute()
                        st.success(f"🎉 成功！商品『{manual_name}』已同步至雲端主檔！")
                        time.sleep(0.8)
                        st.rerun()
                    except Exception as e: st.error(f"❌ 儲存失敗: {str(e)}")
        ...
        with st.form("delete_product_form", clear_on_submit=True):
            ...
            if submit_del_p and del_p_id:
                if input_pwd_p != ADMIN_PASSWORD:
                    st.error("❌ **授權認證失敗**：管理員密碼錯誤。")
                elif not supabase_connected:
                    st.error("❌ **無法刪除**：資料庫未成功連線。")
                else:
                    try:
                        supabase.table("product_master").delete().eq("product_id", del_p_id).execute()
                        st.success(f"🗑️ 已成功自雲端移除商品代號『{del_p_id}』。")
                        time.sleep(0.8)
                        st.rerun()
    ...
    try:
        ...
        if search_query and all_prods:
            filtered_prods = [p for p in all_prods if search_query.lower() in str(p.get("product_id", "")).lower() or search_query in str(p.get("product_name", ""))]
            if filtered_prods:
                ...
                for p in filtered_prods:
                    with st.container():
                        ...
                        with c3: st.markdown(f"📋 **完整規格與成本組合方案：**\n> {p['unit']}")
                        st.markdown("<hr style='border:1px dashed #eee'>", unsafe_allow_html=True)
            else:
                st.warning(f"❌ 查無此商品：找不到任何包含『{search_query}』的編號或品名。")
        ...
        st.error(f"❌ **查詢看板通訊出錯**: {str(query_err)}")
        ...
        with st.form("add_customer_form", clear_on_submit=True):
            ...
            if submit_c:
                if not c_id or not c_name:
                    st.error("❌ **防呆警告**：客戶編號與官方標準全名皆為必填項目！")
                elif not supabase_connected:
                    st.error("❌ **無法儲存**：系統未成功連線至資料庫。")
                else:
                    try:
                        existing_kw = ""
                        try:
                        ...
                        if learned_part: combined_keywords += f" , {learned_part}"
                        elif c_shortcut == "": combined_keywords = ""

                        supabase.table("customers").upsert({
                            "customer_id": c_id, "standard_name": c_name, "search_keywords": combined_keywords
                        }).execute()
                        st.success(f"🎉 成功同步！『{c_name}』的最新資料已成功儲存！")
                        time.sleep(0.8)
                        st.rerun()
                    except Exception as e: st.error(f"❌ 寫入失敗。詳細原因: {str(e)}")
        ...
        with st.form("delete_customer_form", clear_on_submit=True):
            ...
            if submit_d_c and d_c_id:
                if input_pwd_c != ADMIN_PASSWORD:
                    st.error("❌ **授權認證失敗**：管理員驗證密碼錯誤。")
                elif not supabase_connected:
                    st.error("❌ **無法刪除**：系統未成功連線至資料庫。")
                else:
                    try:
                        supabase.table("customers").delete().eq("customer_id", d_c_id).execute()
                        st.success(f"🗑️ 成功！已將客戶編號『{d_c_id}』從雲端徹底刪除！")
                        time.sleep(0.8)
                        st.rerun()
                    except Exception as e: st.error(f"❌ 刪除失敗。詳細原因: {str(e)}")
        ...
        if res and res.data:
            ...
            for row in res.data:
                ...
                parsed_cust_list.append({
                    ...
                    "⭐ 自訂快搜/縮寫": shortcut, "🤖 系統已學會的圖片特徵": line_kw if line_kw else "尚未綁定任何圖片特徵"
                })
    ...
    if isinstance(selected_range, tuple) and len(selected_range) == 2:
        ...
        if supabase_connected:
            try:
                ...
                st.error(f"❌ **大總管資料撈取失敗**：{str(e)}")

        if not raw_orders:
            st.info(f"✨ 報告大總管：在 `{range_label}` 這段時間內，蓄水池目前沒有任何待出貨品項登記。")
        else:
            try:
                ...
                with tab1:
                    ...
                    st.download_button(
                        label="📥 下載《全廠總揀貨備貨單 .csv》", data=csv_summary,
                        file_name=f"全廠總揀貨單_{file_range_tag}.csv", mime="text/csv", use_container_width=True
                    )
                ...
                with tab2:
                    ...
                    st.download_button(
                        label="📥 下載《各客戶分貨明細表 .csv》", data=csv_detail,
                        file_name=f"各客戶分貨明細表_{file_range_tag}.csv", mime="text/csv", use_container_width=True
                    )
            except Exception as total_err:
                st.error(f"❌ **大總管報表生成遭遇致命崩潰**: {str(total_err)}")
    else:
